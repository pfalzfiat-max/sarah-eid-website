...
import { footer as footerFallback, kontakt as kontaktFallback, social as socialFallback } from '@/lib/content';
...
export default function Footer({
  ...
  const footerName = name || footerFallback.name;
  const footerBeruf = berufsbezeichnung || footerFallback.berufsbezeichnung;
  const footerDesc = beschreibung || footerFallback.beschreibung;
  const kontakt = {
    telefon: telefon || kontaktFallback.telefon,
    telefonLink: telefonLink || kontaktFallback.telefonLink,
    email: email || kontaktFallback.email,
    region: region || kontaktFallback.region,
  };

  const socialLinks = [
    {
      label: 'Instagram',
      href: instagram || socialFallback.instagram,
      icon: <InstagramIcon />,
    },
    {
      label: 'Facebook',
      href: facebook || socialFallback.facebook,
      icon: <FacebookIcon />,
    },
    {
      label: 'LinkedIn',
      href: linkedin || socialFallback.linkedin,
      icon: <LinkedInIcon />,
    },
  ].filter((s) => s.href);
            ...
            <address className="not-italic space-y-3">
              <p>
                <a
                  ...
                  className="font-inter text-sm text-cream/40 hover:text-gold transition-colors duration-200"
                  aria-label={`Telefon: ${kontakt.telefon}`}
                >
                  {kontakt.telefon}
                </a>
              </p>
              <p>
                <a
                  ...
                  className="font-inter text-sm text-cream/40 hover:text-gold transition-colors duration-200"
                  aria-label={`E-Mail: ${kontakt.email}`}
                >
                  {kontakt.email}
                </a>
              </p>
